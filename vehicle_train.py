...
def main():
    ...
    paths = [
        params.DATA_training_path_neg, params.DATA_training_path_pos,
        params.VEHICLE_DATA_training_path_pos
    ]
    ...
    sampling_sizes = [
        ...
        params.VEHICLE_training_sample_count_pos
    ]
    ...
    imgs_data = load_images(paths, class_names, sampling_sizes,
                            sample_from_centre,
    ...
    svm.setKernel(
        params.HOG_SVM_kernel)  # use specific kernel type (alteratives exist)

    svm.setC(params.HOG_C_VALUE)
    svm.setGamma(params.HOG_GAMMA_VALUE)
    # svm.setC() 0.1 1 10 100 1000
    # and set gamma 0.1 0.5 0.7 1 1.5 2 5 10
    ...
    svm.trainAuto(
        samples, cv2.ml.ROW_SAMPLE, class_labels, kFold=10, balanced=True)
    ...
    error = (np.absolute(class_labels.ravel() - output).sum()) / float(
        output.shape[0])
    ...
    if error < (1.0 / len(params.DATA_CLASS_NAMES)):
        print("Trained SVM obtained {}% training set error".format(
            round(error * 100, 2)))
        print("-- meaining the SVM got {}% of the training examples correct!".
              format(round((1.0 - error) * 100, 2)))
    else:
        print("Failed to train SVM. {}% error".format(round(error * 100, 2)))

    print_duration(start)

    print(("Finished training BOW detector. {}".format(
        format_time(get_elapsed_time(program_start)))))
